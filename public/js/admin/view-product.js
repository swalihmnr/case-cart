...
function setupBasicInfoEdit() {
    ...
    saveBasicInfoBtn.addEventListener('click', async () => {
        ...
        if (!data.productName || data.productName.length < 3 || data.productName.length > 100) {
            Swal.fire({
                icon: "warning",
                title: "Invalid Input",
...
async function saveVariantChanges() {
    ...
    const originalPrice = parseFloat(document.getElementById('editVariantOriginalPrice').value);
    const salePrice = parseFloat(document.getElementById('editVariantSalePrice').value);
