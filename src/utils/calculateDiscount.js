export default async function discountChecker(
  discIds,
  modelName,
  variantPrice,
  type,
  ...
  for (let id of discIds) {
    ...
    if (discount > bestDiscount) {
      ...
      bestMaxDiscountApplicable =
        promotion.maximumDiscount && promotion.maximumDiscount > 0
  ...
  if (
    bestMaxDiscountApplicable !== null &&
  ...
  console.log(
    `CalculateDiscount: Best calculated discount magnitude is ₹${bestDiscount}`,
  );
