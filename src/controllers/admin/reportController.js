          ...
          totalSales: {
            $sum: {
              $cond: [
                {
                  $in: [
                    { $arrayElemAt: ["$orderItems.status", 0] },
                    ["cancelled", "returned"],
                  ],
                },
                0,
  ...
  try {
    ...
    const tableArray = {
      ...
      rows: orders.map((order) => {
        ...
        const totalRowDiscount =
          (order.totalDiscount || 0) + (order.couponDiscount || 0);
...
const exportExcel = async (req, res) => {
  try {
    ...
    orders.forEach((order) => {
      ...
      const row = sheet.addRow({
        ...
        items: order.orderItems
          ? order.orderItems
              .map((item) => (item.productId ? item.productId.name : "Unknown"))
              .join(", ")
          : "No Items",
    ...
    const sumRow2 = sheet.addRow([
      ...
      null,
      null,
      "Total Successful Sales (Rs):",
      totalSales.toFixed(2),
    ]);
    const sumRow3 = sheet.addRow([
      null,
      null,
      null,
